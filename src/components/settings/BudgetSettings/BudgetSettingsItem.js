...
export default class BudgetSettingsItem extends Component {
  constructor(props) {
    super(props);
    this.state = {value:""}
  }
  ...
  handleEnter = (event) => {
    this.props.handleEnter(this.props.name, this.props.budget.total, Number(this.state.value))
    this.setState({value:""})
  }


  render() {
    let placeholder=this.props.budget.name +": " +this.props.budget.balance + " / " + this.props.budget.total +"$";
    return (
